...
fn main() {
    ...
    // MSVC default stack is 1 MiB. The pager event-loop future overflows it in
    // Windows debug builds (`thread 'main' has overflowed its stack`).
    #[cfg(windows)]
    {
        thread_local! {
            static ON_LARGE_STACK: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
        }
        if !ON_LARGE_STACK.with(std::cell::Cell::get) {
            let join = std::thread::Builder::new()
                .name("grok-main".into())
                .stack_size(16 * 1024 * 1024)
                .spawn(|| {
                    ON_LARGE_STACK.with(|c| c.set(true));
                    main();
                })
                .expect("spawn grok-main with 16MiB stack");
            match join.join() {
                Ok(()) => return,
