...
pub(crate) async fn run(
    ...
    *reader_thread = ReaderThread::spawn(input_tx, input_paused.clone(), reader_parked.clone());
    ...
    let mut grokcraft_rx = xai_grokcraft::hub::global().take_inbound_rx();
    ...
    if app.session_startup_allowed()
        && let Ok(store) = xai_grokcraft::store::load()
        && store.machine_token.as_ref().is_some_and(|t| !t.is_empty())
        && process_effects(
            vec![Effect::GrokcraftConnect { open_browser: false }],
            &mut tasks,
            &mut app,
            &progress_tx,
        )
    ...
    loop {
        ...
        tokio::select! {
            ...
            ev = async {
                match grokcraft_rx.as_mut() {
                    Some(rx) => rx.recv().await,
                    None => std::future::pending().await,
                }
            } => {
                if let Some(ev) = ev {
                    let effs = crate::app::dispatch::grokcraft::handle_event(&mut app, ev);
                    if process_effects(effs, &mut tasks, &mut app, &progress_tx) {
                        break;
                    }
                    schedule_tick(&mut animation_tick_at, &app, tick_interval);
