...
pub enum Action {
    ...
    OpenUrl(String),
    /// Connect this computer to Grokcraft WebUI (`/grokcraft`).
    GrokcraftConnect,
...
pub enum Effect {
    ...
    /// Start (or reuse) the Grokcraft agent WebSocket. `open_browser` is handled in dispatch.
    GrokcraftConnect {
        open_browser: bool,
    },
...
pub enum TaskResult {
    ...
    /// Inbound Grokcraft hub event (pairing, prompt, fold, snapshot request, …).
    Grokcraft(xai_grokcraft::GrokcraftEvent),
}
#[cfg(test)]
mod tests {
    use super::*;
